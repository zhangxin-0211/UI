...
- 默认已注册设备遥测协议：仅接受 `106.55.38.224` 发往本机 `45454` 的心跳与 GCJ-02 位置/艏向报文，并对心跳回复 `ACK`。路线、返航和控制下发编码仍待设备协议确定；在此之前不会发送控制报文。规划成功后可通过 `RoutePage::preparedMission()` 读取任务快照，也会发出 `missionPrepared(const PreparedMission &)` 信号；快照包含真实起点、安全起点、安全目标、往返路径及水域边界障碍点。其他真实算法可实现 `IRoutePlanner` 后通过 `MainWindow::setExternalRoutePlanner()` 替换，未来真实下发协议可通过 `MainWindow::setRobotProtocolCodec()` 注册。
